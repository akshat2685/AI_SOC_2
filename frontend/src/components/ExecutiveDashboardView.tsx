...
const CustomAreaTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-slate-950/95 border border-blue-500/40 backdrop-blur-md p-3 rounded-xl shadow-2xl">
        ...
        <p className="text-[10px] text-slate-500 mt-1 font-medium">Autonomous threat detection telemetry</p>
      </div>
    );
  }
  return null;
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-slate-950/95 border border-indigo-500/40 backdrop-blur-md p-3 rounded-xl shadow-2xl">
        <p className="text-xs font-bold text-slate-200 capitalize">{data.name}</p>
        <div className="flex items-center gap-2 mt-1.5">
        ...
        <p className="text-[10px] text-slate-500 mt-1 font-medium">Distribution of sandbox attack vectors</p>
      </div>
    );
  }
  return null;
