...
const CustomLatencyTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className={`bg-slate-950/95 border ${isSpike ? 'border-amber-500/50' : 'border-slate-800'} backdrop-blur-md p-3 rounded-xl shadow-2xl`}>
        <p className="text-xs font-bold text-slate-300">Time: {label}</p>
        <div className="flex items-center gap-2 mt-1.5">
          <span className={`w-2.5 h-2.5 rounded-full ${isSpike ? 'bg-amber-500 animate-ping' : 'bg-amber-500'}`}></span>
        ...
        <p className="text-[10px] text-slate-500 mt-1 font-medium">
          {isSpike ? '⚠️ Critical latency penalty' : 'Resilient sandbox state'}
        </p>
      </div>
    );
  }
...
const CustomMTTDTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-slate-950/95 border border-emerald-500/40 backdrop-blur-md p-3 rounded-xl shadow-2xl">
        <p className="text-xs font-bold text-slate-300">Time: {label}</p>
        <div className="flex items-center gap-2 mt-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse"></span>
          <p className="text-xs font-semibold text-white">
            <span className="text-slate-400">MTTD Speed: </span>
            {payload[0].value.toFixed(1)} ms
          </p>
        </div>
        <p className="text-[10px] text-slate-500 mt-1 font-medium">Mean Time to Detect (MTTD)</p>
      </div>
    );
  }
  return null;
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-lg h-80">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chaosData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Tooltip content={<CustomMTTDTooltip />} />
